...
impl SysHandler<'_> {
    ...
    pub async fn read_policy(&self, name: &str) -> Result<PolicyInfo, VaultError> {
        self.client
            .exec_with_data(Method::GET, &format!("sys/policies/acl/{}", encode_path(name)), None)
            .await
    }
    ...
    pub async fn write_policy(&self, name: &str, rules: &str) -> Result<(), VaultError> {
        ...
        self.client
            .exec_empty(Method::PUT, &format!("sys/policies/acl/{}", encode_path(name)), Some(&body))
            .await
    }

    pub async fn delete_policy(&self, name: &str) -> Result<(), VaultError> {
        self.client
            .exec_empty(Method::DELETE, &format!("sys/policies/acl/{}", encode_path(name)), None)
            .await
    }
}
