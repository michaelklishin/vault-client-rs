...
async fn approle_destroy_secret_id() {
    ...
    client
        .auth()
        .approle()
        .destroy_secret_id("my-role", &SecretString::from("sec"))
        .await
        .unwrap();
}
...
async fn k8s_login_returns_auth_info() {
    ...
    let auth = client
        ...
        .login(
            "my-role",
            &SecretString::from("eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9.fake"),
        )
        .await
        .unwrap();
    assert_eq!(auth.client_token.expose_secret(), "s.newtoken");
...
async fn token_lookup_posts_token() {
    ...
    let info = client
        .auth()
        .token()
        .lookup(&SecretString::from("s.targettoken"))
        .await
        .unwrap();
...
async fn token_revoke_posts_token() {
    ...
    client
        .auth()
        .token()
        .revoke(&SecretString::from("s.targettoken"))
        .await
        .unwrap();
}
...
async fn k8s_login_updates_internal_token() {
    ...
    client
        .auth()
        .kubernetes()
        .login("my-role", &SecretString::from("fake-jwt"))
        .await
        .unwrap();
