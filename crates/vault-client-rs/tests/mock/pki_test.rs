...
async fn read_role_returns_role_config() {
    ...
    assert_eq!(role.ttl, 86400);
    assert_eq!(role.max_ttl, 259200);
...
async fn revoke_with_key_posts_serial_and_private_key() {
    ...
    let client = build_test_client(&server).await;
    let private_key =
        SecretString::from("-----BEGIN RSA PRIVATE KEY-----\nKEY\n-----END RSA PRIVATE KEY-----");
    let info = client
        .pki("pki")
...
#[tokio::test]
async fn update_issuer_posts_params_and_returns_issuer_info() {
    let server = MockServer::start().await;
    ...
    let params = PkiIssuerUpdateParams {
        issuer_name: Some("renamed-issuer".to_string()),
        leaf_not_after_behavior: Some("truncate".to_string()),
        ..Default::default()
    };
    let info = client
        .pki("pki")
        .update_issuer("issuer-id-1", &params)
        .await
        .unwrap();
    assert_eq!(info.issuer_id, "issuer-id-1");
    assert_eq!(info.issuer_name.as_deref(), Some("renamed-issuer"));
    assert_eq!(info.leaf_not_after_behavior, "truncate");
}

// --- ACME config ---

#[tokio::test]
async fn read_acme_config_returns_config() {
    let server = MockServer::start().await;

    Mock::given(method("GET"))
        .and(path("/v1/pki/config/acme"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "data": {
                "enabled": true,
                "allowed_issuers": ["*"],
    ...
    let client = build_test_client(&server).await;
    let config = client.pki("pki").read_acme_config().await.unwrap();
    assert!(config.enabled);
    assert_eq!(config.allowed_issuers, vec!["*"]);
    assert_eq!(config.allowed_roles, vec!["web-server"]);
    assert_eq!(
        config.default_directory_policy.as_deref(),
        Some("sign-verbatim")
    );
    assert_eq!(config.eab_policy.as_deref(), Some("not-required"));
}

#[tokio::test]
async fn write_acme_config_posts_config() {
    let server = MockServer::start().await;
    ...
    let config = PkiAcmeConfig {
        enabled: true,
        allowed_issuers: vec!["*".to_string()],
        allowed_roles: vec!["web-server".to_string()],
        default_directory_policy: Some("sign-verbatim".to_string()),
        dns_resolver: None,
        eab_policy: Some("not-required".to_string()),
    };
    client.pki("pki").write_acme_config(&config).await.unwrap();
}

// --- Delta CRL ---

#[tokio::test]
async fn rotate_delta_crl_uses_post_method() {
    let server = MockServer::start().await;

    Mock::given(method("POST"))
        .and(path("/v1/pki/crl/rotate-delta"))
        .respond_with(ResponseTemplate::new(204))
        .expect(1)
        .mount(&server)
        .await;
    ...
    let client = build_test_client(&server).await;
    client.pki("pki").rotate_delta_crl().await.unwrap();
}

// --- Raw CRL ---

#[tokio::test]
async fn read_crl_returns_raw_bytes() {
    let server = MockServer::start().await;

    let crl_bytes: Vec<u8> = vec![0x30, 0x82, 0x01, 0x0A, 0x30, 0x81, 0xB1];
    ...
    let client = build_test_client(&server).await;
    let result = client.pki("pki").read_crl().await.unwrap();
    assert_eq!(result, crl_bytes);
}

#[tokio::test]
async fn read_crl_delta_returns_raw_bytes() {
    let server = MockServer::start().await;

    let delta_bytes: Vec<u8> = vec![0x30, 0x82, 0x02, 0xFF, 0xAA, 0xBB];

    Mock::given(method("GET"))
        .and(path("/v1/pki/crl/delta"))
        .respond_with(ResponseTemplate::new(200).set_body_bytes(delta_bytes.clone()))
        .expect(1)
        .mount(&server)
        .await;

    let client = build_test_client(&server).await;
    let result = client.pki("pki").read_crl_delta().await.unwrap();
    assert_eq!(result, delta_bytes);
}

// --- Cross-signing ---

#[tokio::test]
async fn cross_sign_intermediate_posts_to_correct_path() {
    let server = MockServer::start().await;

    Mock::given(method("POST"))
        .and(path("/v1/pki/intermediate/cross-sign"))
        .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
            "data": {
                "certificate": "-----BEGIN CERTIFICATE-----\nCROSS-SIGNED\n-----END CERTIFICATE-----",
                "issuing_ca": "-----BEGIN CERTIFICATE-----\nCA\n-----END CERTIFICATE-----",
    ...
    let client = build_test_client(&server).await;
    let params = PkiCrossSignRequest {
        common_name: "Cross-Signed Intermediate".to_string(),
        ..Default::default()
    };
    let cert = client
        .pki("pki")
        .cross_sign_intermediate(&params)
        .await
        .unwrap();
    assert_eq!(cert.serial_number, "cc:dd:ee:ff:00");
    assert!(cert.certificate.contains("CROSS-SIGNED"));
