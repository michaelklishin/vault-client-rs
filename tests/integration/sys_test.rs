...
use vault_client_rs::{Kv2Operations, TokenAuthOperations, VaultClient};
...
async fn wrap_lookup_and_unwrap() {
    let wrapping_client = build_wrapping_client("5m");
    let client = build_client();

    // Create a token with wrapping — the response will be wrapped
    let params = TokenCreateRequest {
        policies: Some(vec!["default".into()]),
        ttl: Some("1h".into()),
        ..Default::default()
    ...
    // Let's use a different approach — use the sys.wrap_lookup path.
    // We'll create a secret and wrap-read it.
    let kv = wrapping_client.kv2("secret");
    let path = unique_name("wrap");
    // First write a secret with the normal client
    let normal_client = build_client();
    normal_client
        .kv2("secret")
    ...
    // Reading with wrapping client should give us a wrapping token
    // But our API unwraps automatically. Let's test wrap_lookup with a direct approach.
    // We can test that the wrapping client was constructed properly
...
async fn wrap_rewrap() {
    ...
    let client = build_wrapping_client("5m");
