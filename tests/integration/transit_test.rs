...
async fn encrypt_decrypt() {
    ...
    let ct = transit
        .encrypt(&name, &SecretString::new("sensitive data".into()))
        .await
        .unwrap();
...
async fn rotate_and_rewrap() {
    ...
    let ct_v1 = transit
        .encrypt(&name, &SecretString::new("hello".into()))
        .await
        .unwrap();
...
async fn batch_encrypt_decrypt() {
    ...
    let items = vec![
        TransitBatchPlaintext {
            plaintext: B64.encode("first"),
            context: None,
        },
        TransitBatchPlaintext {
            plaintext: B64.encode("second"),
            context: None,
        },
    ];
...
async fn rewrap_preserves_plaintext() {
    ...
    let ct = transit
        .encrypt(&name, &SecretString::new(original.into()))
        .await
        .unwrap();
