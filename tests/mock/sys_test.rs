...
async fn unseal_sends_key() {
    ...
    let status = client
        .sys()
        .unseal(&SecretString::new("unseal-key".into()))
        .await
        .unwrap();
    assert!(!status.sealed);
}
...
async fn unwrap_posts_token() {
    ...
    let data: HashMap<String, String> = client
        .sys()
        .unwrap(&SecretString::new("s.wrapped".into()))
        .await
        .unwrap();
...
async fn wrap_lookup_returns_wrap_info() {
    ...
    let info = client
        .sys()
        .wrap_lookup(&SecretString::new("s.wrapped-token".into()))
        .await
        .unwrap();
...
async fn capabilities_returns_map() {
    ...
    let caps = client
        .sys()
        .capabilities(
            &SecretString::new("s.my-token".into()),
