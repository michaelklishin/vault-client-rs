...
async fn userpass_login() {
    ...
    let auth = client
        .auth()
        .userpass()
        .login("alice", &SecretString::new("s3cret".into()))
        .await
        .unwrap();
...
async fn userpass_update_password() {
    ...
    client
        .auth()
        .userpass()
        .update_password("alice", &SecretString::new("newpass".into()))
        .await
        .unwrap();
}
...
async fn ldap_login() {
    ...
    let auth = client
        .auth()
        .ldap()
        .login("jdoe", &SecretString::new("ldappass".into()))
        .await
        .unwrap();
    assert_eq!(auth.client_token.expose_secret(), "s.newtoken");
    assert_eq!(auth.accessor, "acc-new");
...
async fn github_login() {
    ...
    let auth = client
        .auth()
        .github()
        .login(&SecretString::new("ghp_abc123".into()))
        .await
        .unwrap();
    assert_eq!(auth.client_token.expose_secret(), "s.newtoken");
    assert_eq!(auth.accessor, "acc-new");
...
async fn oidc_login_jwt() {
    ...
    let auth = client
        ...
        .login_jwt(
            "my-role",
            &SecretString::new("eyJhbGciOiJSUzI1NiJ9.fake".into()),
        )
        .await
        .unwrap();
    assert_eq!(auth.client_token.expose_secret(), "s.newtoken");
...
async fn oidc_configure() {
    ...
    let config = OidcConfigRequest {
        ...
        oidc_client_secret: Some("client-secret-456".into()),
